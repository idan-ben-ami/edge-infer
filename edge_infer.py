...
class PredictEmitter:
    ...
    def __init__(self, ops: list[dict], weights: dict[str, np.ndarray],
        ...
        # Sum of all activation buffer sizes (bytes). Pessimistic
        # stack-budget upper bound — ignores lifetime overlap and slot
        # reuse, so the real measured stack from `stack_probe` is
        # typically lower. Used by lib.rs to expose
        # PEAK_ACTIVATION_BYTES_UPPER_BOUND so users can compile-time
        # check the model fits their target's RAM.
        self.activation_bytes_total: int = 0
    ...
    def _emit_conv(self, op: dict):
        ...
        self.activation_bytes_total += co * oh * ow * 4
    ...
    def _emit_maxpool(self, op: dict):
        ...
        self.activation_bytes_total += c * oh * ow * 4
        self.lines.append(
    ...
    def _emit_reshape(self, op: dict):
        ...
        # Strip a leading `*` if input_var is a pre-dereffed flat ref;
        # otherwise method dispatch autoderefs through both & and *.
        base = input_var[1:] if input_var.startswith("*") else input_var
        # The function parameter `input` always has a rank-3 Rust type
        # (CHW), even when the ONNX shape is rank-4 (NCHW with batch dim
        # stripped at the signature). For internal tensors, the Rust
        # nesting matches the tracker shape exactly.
        rank = len(input_shape)
        if input_var == "input" and rank == 4:
            rank = 3
        flatten_chain = ".as_flattened()" * max(0, rank - 1)
        ...
        self.lines.append(
            f"    let flat: &[f32; {flat_size}] = "
            f"{base}.as_slice(){flatten_chain}.try_into().unwrap();"
        )
    ...
    def _emit_flatten(self, op: dict):
        ...
        base = input_var[1:] if input_var.startswith("*") else input_var
        # See _emit_reshape: the function parameter `input` has rank-3
        # Rust type even when ONNX shape is rank-4 (batch stripped).
        rank = len(input_shape)
        if input_var == "input" and rank == 4:
            rank = 3
        flatten_chain = ".as_flattened()" * max(0, rank - 1)
    ...
    def _emit_gemm(self, op: dict):
        ...
        self.activation_bytes_total += out_features * 4
...
def emit_lib_rs(ops: list[dict], weights: dict[str, np.ndarray],
    ...
    activation_upper_bound = emitter.activation_bytes_total
    ...
    code = textwrap.dedent(f"""\
        ...
        /// );
        /// ```
        pub const PEAK_ACTIVATION_BYTES_UPPER_BOUND: usize = {activation_upper_bound};
